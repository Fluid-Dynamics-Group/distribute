...
async fn check_deallocate_jobs() {
    ...
    let server = Server::new(nodes_file, server_save_dir.clone(), server_temp_dir.clone(), server_port, false);
    ...
    let run = Add::new("./tests/apptainer_local/distribute-jobs.yaml".into(), server_port, addr, false, false);
