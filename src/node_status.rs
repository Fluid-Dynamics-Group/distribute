...
pub async fn node_status(args: cli::NodeStatus) -> Result<(), Error> {
    ...
    for node in nodes_config.nodes {
        let c = tokio::net::TcpStream::connect(node.keepalive_addr()).await.map_err( |e| {
            results.push(StatusResponse::new(node.node_name.clone(), node.keepalive_addr(), Status::Offline));
            error::TcpConnection::from(e)
        })?;
        let mut connection : Connection<transport::ServerQuery> = Connection::from_connection(c);

        connection.transport_data(&transport::ServerQuery::VersionCheck).await.map_err(error::TcpConnection::from)?;
        ...
        match connection.receive_data().await.unwrap() {
            ...
            transport::ClientQueryAnswer::VersionResponse(version) => {
                results.push(StatusResponse::new(node.node_name.clone(), node.keepalive_addr(), Status::from(version)));
            }
        }
    }
