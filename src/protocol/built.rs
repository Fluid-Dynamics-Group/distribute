...
use super::executing::{Executing, ClientExecutingState, ServerExecutingState};
...
impl Machine<Built, ClientBuiltState> {
    ...
    pub(crate) async fn get_execute_instructions(mut self) -> Result<super::ClientEitherPrepareBuild<Machine<Executing, ClientExecutingState>>, Error> {
        ...
        let build_job : server::pool_data::BuildTaskInfo= match job {
            server::pool_data::FetchedJob::Build(build) => {
                if build.identifier == self.state.job_identifier {
                    error!("scheduler returned a build instruction for a job we have already compiled on {} / {} This is a bug", self.state.common.node_name, self.state.conn.addr );
                    panic!("scheduler returned a build instruction for a job we have already compiled on {} / {} This is a bug", self.state.common.node_name, self.state.conn.addr );
                } else {
                    // notify the compute machine that we are transitioning states
                    self.state.conn.transport_data(&ServerMsg::ReturnPrepareBuild).await?;
            ...
            server::pool_data::FetchedJob::Run(run) => {
                self.state.conn.transport_data(&ServerMsg::ExecuteJob(run.task)).await?;
